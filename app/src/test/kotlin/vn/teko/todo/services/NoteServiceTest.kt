...
internal class NoteServiceTest {
    ...
    fun setUp() {
        ...
        val notes = listOf<NoteModel>(note1, note2, note3).asFlow()
        runBlocking {
            given(noteRepository.findAll()).willReturn(notes)
            given(noteRepository.findById(1)).willReturn(note1)
            given(noteRepository.findById(2)).willReturn(note2)
            given(noteRepository.findById(3)).willReturn(note3)
            ...
            given(colorRepository.findById(1)).willReturn(color1.toColorModel())
            ...
            given(labelRepository.findById(1)).willReturn(label1.toLabelModel())
            given(labelRepository.findById(2)).willReturn(label2.toLabelModel())
            ...
            given(labelRepository.findByNoteId(4)).willReturn(listOf<Label>(label2).map { it.toLabelModel() })
